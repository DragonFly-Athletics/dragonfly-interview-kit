...
- Tooling note: the proctor may capture timestamps from terminal
  scrollback after the session to ground Dimensions 4, 6, and 10. Nothing
  is recorded. Only the live session counts as observation.

## How this maps to the engineering screen

This session is not a pass/fail coding test. It exists to produce
evidence on the traits DragonFly screens for, which a résumé and a
conversation cannot show.

- **Tolerance for ambiguity.** The fill-rate metric is deliberately
  under-specified. Dimensions 1 and 5 read how you close that gap.
- **Enough Node to read the code and judge how it is organised.**
  You inherit a codebase you did not write. Dimensions 2 and 8 read how
  fast you orient in it and what you do to its shape.
- **Systems design, contention, and atomic operations.** The advance
  endpoint guards one official against two overlapping games, and it
  touches shared state to do it. Dimension 8 reads whether you notice.
- **Calibrated trust in agent output.** Accepting every line and reading
  every line are both failures. Dimensions 3 and 4 read the calibration
  between them, and Dimension 7 reads it again at a distance.
